...
### PWA requirements
- Installable on iOS, Android, desktop (manifest + icons + service worker)
...
1. **iOS Safari does NOT fire `beforeinstallprompt`.** The install affordance on iOS must be a custom "Add to Home Screen" hint with screenshot/illustration of the Share → Add to Home Screen flow. Detect iOS in `InstallPrompt.jsx` and branch UI accordingly. Do not try to programmatically install on iOS — it cannot be done.
2. **Admin panel is online-only.** `/admin` routes (Phase 7 — import wizard, editable table, image upload) must check `navigator.onLine` and show a "You're offline — admin requires connection" screen. Reason: bulk imports and image uploads are footguns offline (partial writes, lost uploads, sync conflicts). The user-facing builder remains fully offline-capable.
3. **iOS storage cap.** iOS PWAs have a ~50 MB IndexedDB quota and can be evicted after 7 days of inactivity. Keep cached unit catalogue lean; do not cache full-size images for offline use, only thumb/card variants.
4. **Standalone display mode hides browser chrome.** Build an in-app top bar with back/forward only where it would meaningfully replace browser nav — don't reinvent the address bar.
